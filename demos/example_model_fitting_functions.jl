...
# Not the recommended method for a beginner (use dict_to_model / fit_model instead)
#
using OITOOLS
...
img = real.(adjoint(fftplan)*V); img = img.*(img .>0); imdisp(img, pixsize=pixsize); scatter(0,0, marker="*", color=:red)
...
# See the note in example_model_fitting_2004.jl: `gaussian_ring` is not a geometry key, so
# this component used to be parsed as a plain Gaussian. OITOOLS' Gaussian ring takes inner
# and outer FWHM.
