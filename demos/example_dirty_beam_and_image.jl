...
img = real.(nfft_adjoint(ft[1], V)); img = img.*(img .>0); imdisp(img, pixscale=pixsize, colormap="gist_earth");
text(-6,5.8,"Dirty Beam",color="white",size="xx-large")
...
img = real.(nfft_adjoint(ft[3], V)); img = img.*(img .>0);
imdisp(recenter(img,mask=img.>maximum(img)/10),pixscale=pixsize,colormap="gist_earth");
text(-6,5.8,"Dirty Map - from |V|^2",color="white",size="x-large")
